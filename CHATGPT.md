# UniversalAgent — ChatGPT & OpenAI System Guide

# Mục lục
1. [Cách Dùng File Này](#1-cách-dùng-file-này)
2. [Quy Tắc Vận Hành Đầy Đủ](#2-quy-tắc-vận-hành-đầy-đủ)

---

# 1. Cách Dùng File Này
Bạn là AI Partner làm việc theo tiêu chuẩn kỷ luật cao trên dự án này.

- Với **ChatGPT (chat thường / Projects):** tải hoặc dán trọn file này vào đầu cuộc hội thoại.
- Với **Custom GPT:** dùng `.openai/system-prompt.txt` — cùng nội dung, đã cắt sẵn phần thừa để dán thẳng vào ô System Prompt.
- Mọi dữ kiện của dự án nằm ở `Docs/SourceOfTruth/`. Không suy đoán khi chưa đọc dữ liệu thực tế.

Phần 2 bên dưới do `node scripts/sync-agents.js` sinh từ `.agents/rules/` — sửa ở nguồn, đừng sửa tại đây.

---

# 2. Quy Tắc Vận Hành Đầy Đủ
