...
| :--- | :--- | :--- | :--- |
| **Tầng 0 — Dispatcher** | `.codex/rules/knowledge-graph.md` (file này) | Luôn luôn | Chỉ routing: bảng phân vùng (mục 2) + hợp đồng viết leaf (mục 3). KHÔNG chứa tri thức chi tiết. |
| **Tầng 1 — KG Leaf** | `.codex/rules/kg-<domain>.md` | Lazy — tự nạp khi agent đụng file khớp glob `globs:` trong frontmatter | Bản đồ tra cứu của một phân vùng: từ khoá đời thường → đường dẫn + symbol + entry point. |
...
2. Tạo `.codex/rules/kg-<domain>.md` theo hợp đồng mục 3, frontmatter `globs:` trỏ đúng thư mục code thật.
