...
//
// Mục đích: đo độ dài file tài liệu (.md/.txt trong Docs/, .claude/, .agents/, .ai/ hoặc CLAUDE.md/AGENTS.md)
// vừa được ghi, so với hạn mức theo "class nạp" của file đó.

const fs = require('fs');
const path = require('path');
const { execFileSync } = require('child_process');
const { parseContext, normalizePath } = require('./hook-adapter');
...
function scanAlwaysOnSurface(root) {
  ...
  for (const name of entries) {
    ...
    files.push({ rel: '.ai/rules/' + name, content });
  }
...
function checkHookRegistry(root) {
  ...
  const hooksDir = path.join(root, '.ai', 'hooks');
  let hookScripts = [];
  try {
    hookScripts = fs.readdirSync(hooksDir)
      .filter((name) => (name.endsWith('.js') || name.endsWith('.mjs')) && name !== 'hook-adapter.js')
  ...
  for (const abs of hookScripts) {
    ...
    if (!referenced) {
      console.error(
        `[DOC BUDGET] LỖI — ${base} nằm trong .ai/hooks/ nhưng không thấy khai trong .claude/settings.json hoặc .agents/hooks.json.`
      );
      hasError = true;
    }
  }
...
function main() {
  ...
  const filePath = context.filePaths.length > 0 ? context.filePaths[0] : null;

  if (!filePath) {
    if (context.engine === 'antigravity') console.log('{}');
    process.exit(0);
    return;
  }
